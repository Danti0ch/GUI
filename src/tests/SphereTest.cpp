...
static const geom::Vector3D BEHOLDER_VTOR(20, 40, 60);
static const double   AMBIENT_VAL        = 0.6;
static const uint     SPECULAR_POW_RATIO = 5;
static const double   LIGHT_Z_CT         = 100;

static const gglib::Color SPHERE_COLOR(255, 100, 40, 255);
static const gglib::Color LIGHT_COLOR(100, 123, 86, 255);

static const double DIFFUSE_RATIO   = 1.1;
...
SphereRender::SphereRender():gglib::GraphicSpace(WINDOW_WIDTH, WINDOW_HEIGHT),
    image_cts_(10, 710, 10, 710, -100, 100, -100, 100),
    manip_cts_(720, 920, 400, 600, -100, 100, -100, 100),
    x_manip_(30), y_manip_(30)
{
    Clear();
    manip_cts_.Draw(this, gglib::Color(0, 0, 0, 255));

    gglib::Vector click_vtor(x_manip_, y_manip_, gglib::VT_DATA::COORD);
...
void SphereRender::draw_sphere(){
    ...
    uint y_min = image_cts_.y_lower_pixel();
    uint y_max = image_cts_.y_upper_pixel();
    ...
    for(uint y_i = y_min; y_i <= y_max ; y_i++){
        for(uint x_i = x_min; x_i <= x_max; x_i++){
            ...
            gglib::Color cur_color = get_sphere_pixel_color(x_i, y_i);
...
gglib::Color SphereRender::get_sphere_pixel_color(uint x_pix, uint y_pix){
    ...
    if(z_real_quad <= geom::EPS) return gglib::Color(255, 255, 255, 255);

    double z_real = sqrt(z_real_quad);

    geom::Vector3D to_point_vtor(x_real, y_real, z_real);
    geom::Vector3D light_vtor(x_manip_, y_manip_, LIGHT_Z_CT);

    geom::Vector3D light_to_point_vtor    = light_vtor - to_point_vtor;
    geom::Vector3D beholder_to_point_vtor = BEHOLDER_VTOR - to_point_vtor;
    
    double cosA = geom::CountCosAngle(to_point_vtor, light_to_point_vtor);
    ...
    if(cosA > geom::EPS){
        ...
        geom::Vector3D light_reflected_vtor = to_point_vtor * 2 - light_to_point_vtor;
    ...
    gglib::Color tmp_color    = SPHERE_COLOR * LIGHT_COLOR;
    gglib::Color result_color = tmp_color * diffuse_val * DIFFUSE_RATIO +
...
void SphereRender::MouseButtonPressHandler(uint x_pos, uint y_pos){
    ...
    if(gglib::CheckCoordInCTS(manip_cts_, x_pos, y_pos)){

        x_manip_ = manip_cts_.CountAxisPosX(x_pos);
        y_manip_ = manip_cts_.CountAxisPosY(y_pos);
    }
    else if(gglib::CheckCoordInCTS(image_cts_, x_pos, y_pos)){
